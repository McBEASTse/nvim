...
-- Autocompile SCSS to CSS
vim.api.nvim_create_autocmd('BufWritePost', {
  pattern = '*.scss',
  callback = function()
    -- Get the current file path and create the CSS output path
    local scss_file = vim.fn.expand '%:p'
    local css_file = vim.fn.expand '%:p:r' .. '.css'

    -- Run Sass compilation silently and capture any errors
    local output = vim.fn.system(string.format('sass --no-source-map %s %s 2>&1', scss_file, css_file))

    -- Only show output if there are erros (Sass returns non-zero exit code)
    if vim.v.shell_error ~= 0 then
      -- Splut the error message into lines
...
require('lazy').setup({
  ...
  { -- Autoformat
        ...
        function()
          require('conform').format {
            ...
            callback = function(err)
              if err then
                vim.notify('Formatting failed: ' .. vim.inspect(err), vim.log.levels.ERROR, {
    ...
    opts = {
      ...
      -- Single attributes per line (HTML)
      formatters = {
        prettierd = {
  ...
  { -- You can easily change to a different colorscheme.
    ...
    config = function()
      ...
      vim.api.nvim_set_hl(0, 'StatusLine', { bg = '#2d2d2d' })
      vim.api.nvim_set_hl(0, 'StatusLineNC', { bg = '#222222' })
    end,
  },
